...
def generate_launch_description():
    ...
    # docker_1_command = ExecuteProcess(
    #     cmd=['docker', 'run', '-it', '--rm', '-v', '/dev:/dev', '--privileged', '--net=host', 'microros/micro-ros-agent:humble', 'serial', '--dev', '/dev/ttyACM0', '-b', '115200']
    # )
    # ld.add_action(docker_1_command)
    # docker_2_command = ExecuteProcess(
    #     cmd=['docker', 'run', '-it', '--rm', '-v', '/dev:/dev', '--privileged', '--net=host', 'microros/micro-ros-agent:humble', 'serial', '--dev', '/dev/ttyACM1', '-b', '115200']
    # )
    # ld.add_action(docker_2_command)

    # Add the second command: ros2 launch rplidar_ros rplidar.launch.py
    rplidar_command = ExecuteProcess(
        cmd=['ros2', 'launch', 'rplidar_ros', 'rplidar.launch.py']
    )
    ld.add_action(rplidar_command)

    # Add the third command: ros2 launch toto2_desc slam.launch.py
    ros2_command = ExecuteProcess(
        cmd=['ros2', 'launch', 'toto2_desc', 'nodes.launch.py']
    )
