...
def generate_launch_description():
    ...
    view_rplidar_node = ExecuteProcess(
        cmd=['ros2', 'launch', 'rplidar_ros2', 'view_rplidar.launch.py']
    )
    ld.add_action(view_rplidar_node)

    # Add the fourth command: ros2 launch toto2_desc robot.launch.py
    view_robot_node = ExecuteProcess(
        cmd=['ros2', 'launch', 'toto2_desc', 'robot.launch.py']
    )
